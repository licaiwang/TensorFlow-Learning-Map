...
先從最簡單的只有一個參數w入手，定義 w^* = arg\ \underset{x}{\operatorname{\min}} L(w)
...
解釋完單個模型參數w，引入2個模型參數w 和b ， 其實過程是類似的，需要做的是偏微分，過程如圖9所示，偏微分的求解結果文章後面會有解釋，詳細的求解過程自行Google。
...
其實還是線性模型，因為把x_{cp}^1 = (x_{cp})^2 看作一個特徵，那麼y = b + w_1·x_{cp} + w_2·x_{cp} ^1 其實就是線性模型。
...
- w 越小，表示 function 較平滑的， function輸出值與輸入值相差不大
